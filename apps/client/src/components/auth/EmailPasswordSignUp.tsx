  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.error) {
        switch (res.status) {
          case 400:
            if (res.error?.includes("invite code")) {
              setErrors({ name: "", email: "", password: "", inviteCode: res.error || "Invalid invite code" })
            } else {
              setErrors((prev) => ({
                ...prev,
                email: res.error || "Failed to create account",
              }))
            }
            break
          case 409:
          ...
          default:
            setErrors((prev) => ({
              ...prev,
              email: res.error || "Sign up failed",
            }))
        }
      } else {

...
import { useUpdateNoteContent } from "@/features/notes/mutations"
import { Button } from "../ui/button"
import { collabTokenQueryOptions } from "@/features/notes/queries"
...
const getDisplayName = (session: Session | null, fallbackId: string) => {
  return (
    session?.user?.name ??
    session?.user?.email ??
...
export default function NoteEditor({
  ...
  const { data: session, isLoading: sessionPending } = useQuery(sessionQueryOptions)
  const updateNoteContentMutation = useUpdateNoteContent()

  // Collab is always available (inferred from origin)
  const isManualMode = false

  // Fetch collab token for collaborative editing
  const { data: collabToken, isLoading: collabTokenLoading, error: collabTokenError } = useQuery({
    ...collabTokenQueryOptions(workspaceSlug, notePublicId),
    enabled: !!workspaceSlug && !!notePublicId && !sessionPending && !!session?.user,
  })
  ...
  // Get validated search params from TanStack Router
  const search = useSearch({
    from: "/_authed/workspaces/$workspaceSlug/notes/$noteId/"
  })

  // Helper to validate view mode from search params
  const isValidViewMode = (value: string | undefined): value is ViewMode => {
    return value === "code" || value === "preview" || value === "split"
  }

  const [viewMode, setViewMode] = useState<ViewMode>(() => {
    return isValidViewMode(search.view) ? search.view : "split"
  })
  ...
  // Sync viewMode when search params change
  useEffect(() => {
    setViewMode(isValidViewMode(search.view) ? search.view : "split")
  }, [search.view, isValidViewMode])
  ...
  const updateViewMode = useCallback(
    ...
    [navigate, workspaceSlug, notePublicId]
  )

  const labelMode = useMemo(() => {
    return search.labels === "name" ? "name" : "display"
  }, [search.labels])
  ...
  useEffect(() => {
    applyAwareness(providerRef.current, session ?? null, awarenessColor)
    updateActiveUsers()
  ...
  useEffect(() => {
    ...
    const abortController = new AbortController()
      ...
      try {
        ...
        // Wait for collab token to be available from React Query
        if (collabTokenLoading) {
          return
        }

        if (collabTokenError) {
          throw new Error(collabTokenError instanceof Error ? collabTokenError.message : "Failed to fetch collab token")
        }

        if (!collabToken) {
          throw new Error("Collab token not available")
        }
        ...
        // Collab is always available and inferred from origin
        const wsUrl = new URL("/ws", window.location.origin)
        wsUrl.protocol = wsUrl.protocol === "https:" ? "wss:" : "ws:"

        const provider = new WebsocketProvider(
          wsUrl.toString(),
          `${workspaceId}/${notePublicId}`,
          doc,
          {
            params: { token },
          }
        ...
        applyAwareness(provider, session ?? null, awarenessColor)
        ...
        const connectionTimeout = setTimeout(() => {
          if (isActive && status === "connecting") {
            setError("Connection timeout - check if collab server is running on port 8787")
  ...
  const handleManualSave = useCallback(async () => {
    if (!isManualMode || updateNoteContentMutation.isPending || !hasLocalEdits) {
      return
    }
    ...
    try {
      const data = await updateNoteContentMutation.mutateAsync({
        workspaceSlug,
        notePublicId,
        content,
        includeBlame: true,
  ...
  }, [content, hasLocalEdits, isManualMode, notePublicId, updateNoteContentMutation, workspaceSlug])
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          {isManualMode && (
            <Button
              ...
              disabled={updateNoteContentMutation.isPending || !hasLocalEdits}
              className="h-6 px-2 text-[11px]"
            >
              {updateNoteContentMutation.isPending ? (
                <>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={() => navigate({
                  to: '/workspaces/$workspaceSlug/notes/$noteId/history',
                  params: { workspaceSlug, noteId: notePublicId },
